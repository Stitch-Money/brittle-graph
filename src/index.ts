...
type FieldDefinition<Context> =
    (ctx: Context, node: NodeState, arg: any) => any
...
type InferField<Context, Nodes extends GraphNodes<Context, Nodes>, Node extends GraphNode<Context, Nodes>, Field extends Node['fields']> =
    Field extends ((ctx: Context, node: NodeState, arg: infer Arg) => infer Result)
    ? (arg: Arg) => Result
    : (
        Field extends ((ctx: Context, node: NodeState) => infer Result)
        ? () => Result
        : never
    );
...
const graph = build({
    FROG: {
        ...
        fields: {
            legs: (ctx: {}, nodeState: {}, arg: { includeToes: boolean }) => 4
        }
    },
    CATS: {
        fields: {
            claws: (ctx: {}, nodeState: {}, arg: { includeBacklegs: boolean }) => arg.includeBacklegs ? 20 : 10,
            legs: (ctx: {}, nodeState: any, arg: { includeToes: boolean, includeBacklegs: boolean }) => arg.includeBacklegs ? 4 : 2
        },
        edges: {
...
async function testFunction() {
    const frog = await graph.FROG({ name: 'benny' });
    const result = frog.fields.legs({ includeToes: true });
}